...
def calculate_full_score_safe(ticker):
    ...
    try:
        ...
        # --- A. 配当計算 (空データ・IPO銘柄対策版) ---
        growth_years = 0
        d_cagr = 0
        latest_div_sum = 0
        confirmed_div = pd.Series()

        if not divs.empty:
            # 年間集計
            yearly_div = divs.resample("YE").sum()
            # IndexがDatetime型かつ'year'属性を持つかチェック (130A.T等のエラー対策)
        ...
        # --- C. 数値の算出 (自前計算を優先して info への依存を減らす) ---
        # 1. 株価取得 (利回り計算用)
